...
int do_config(void)
{
    ...
    dp.wintitle = dupprintf("%s 重新配置", appname);//"Configuration"
    dp.errtitle = dupprintf("%s 错误", appname);//"Error"
    dp.data = conf;
...
int do_reconfig(HWND hwnd, int protcfginfo)
{
    ...
    dp_add_tree(&dp, &ctrls_base);
    dp_add_tree(&dp, &ctrls_panel);
    dp.wintitle = dupprintf("%s 重新配置", appname);//"Reconfiguration"
    dp.errtitle = dupprintf("%s 错误", appname);//"Error"
    dp.data = conf;
    dlg_auto_set_fixed_pitch_flag(&dp);
    dp.shortcuts['g'] = TRUE;	       /* the treeview: `Cate&gory' */
...
int askalg(void *frontend, const char *algtype, const char *algname,
	   void (*callback)(void *ctx, int result), void *ctx)
{
    static const char mbtitle[] = "%s 安全警报";//"Security Alert"
	      ...
	      void (*callback)(void *ctx, int result), void *ctx)
{
	...
	"或点击`Cancel`禁用日志记录."; //"or Cancel to disable logging"
    char *message;
    char *mbtitle;
    int mbret;

    message = dupprintf(msgtemplate, FILENAME_MAX, filename->path);
    mbtitle = dupprintf("%s 输出日志到文件", appname);//"Log to File"
