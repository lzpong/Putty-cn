...
static void start_backend(void)
{
    ...
    if (back == NULL) {
	char *str = dupprintf("%s 内部错误", appname); //"Internal Error"
	MessageBox(NULL, "遇到不支持的协议号", //"Unsupported protocol number found"
...
static void close_session(void *ignored_context)
{
    ...
    session_closed = TRUE;
    sprintf(morestuff, "%.70s (非活动)", appname); //"inactive"
	...
	InsertMenu(popup_menus[i].menu, IDM_DUPSESS, MF_BYCOMMAND | MF_ENABLED,
		   IDM_RESTART, "&R重启会话");//"&Restart Session"
    }
}
...
int WINAPI WinMain(HINSTANCE inst, HINSTANCE prev, LPSTR cmdline, int show)
{
    ...
    if (!init_winver())
    {
	char *str = dupprintf("%s 致命错误", appname); //"Fatal Error"
	MessageBox(NULL, "Windows拒绝报告版本", //"Windows refuses to report a version"
		   str, MB_OK | MB_ICONEXCLAMATION);
	sfree(str);
	return 1;
    }
    ...
    if (hr != S_OK && hr != S_FALSE) {
        char *str = dupprintf("%s 致命错误", appname); //"Fatal Error"
	MessageBox(NULL, "无法初始化COM子系统", //"Failed to initialize COM subsystem"
		   str, MB_OK | MB_ICONEXCLAMATION);
	sfree(str);
	return 1;
    }
	...
	if (*p == '@') {
		...
		if (ret == -2) {
		     ...
		     */
		    char *s1, *s2;
		    s1 = dupprintf("此过程将删除与%s相关联的所有注册表项,\n" //"This procedure will remove ALL Registry entries\n"
				   "并且还将删除随机种子文件.\n" //"associated with %s, and will also remove\n"
				   "(这仅影响当前登录的用户)\n" //"the random seed file. (This only affects the\n"
				   //"currently logged-in user.)\n"
				   "\n"
				   "这个过程会删除你保存的会话.\n" //"THIS PROCESS WILL DESTROY YOUR SAVED SESSIONS.\n"
				   "你真的确定要继续吗？", //"Are you really sure you want to continue?",
				   appname);
		    s2 = dupprintf("%s 警告", appname); //"Warning"
	...
	AppendMenu(popup_menus[CTXMENU].menu, MF_ENABLED, IDM_PASTE, "粘贴\t(&P)"); //"&Paste"
	...
	for (j = 0; j < lenof(popup_menus); j++) {
	    ...
	    AppendMenu(m, MF_ENABLED, IDM_SHOWLOG, "事件日志\t(&E)"); //"&Event Log"
	    AppendMenu(m, MF_SEPARATOR, 0, 0);
		AppendMenu(m, MF_ENABLED, IDM_NEWSESS, "新建会话\t(&W)"); //"Ne&w Session..."
	    AppendMenu(m, MF_ENABLED, IDM_DUPSESS, "复制会话\t(&D)"); //"&Duplicate Session"
	    AppendMenu(m, MF_POPUP | MF_ENABLED, (UINT_PTR) savedsess_menu,
		       "保存会话\t(&V)"); //"Sa&ved Sessions"
	    AppendMenu(m, MF_ENABLED, IDM_RECONF, "更改设置\t(&G)"); //"Chan&ge Settings..."
	    AppendMenu(m, MF_SEPARATOR, 0, 0);
	    AppendMenu(m, MF_ENABLED, IDM_COPYALL, "全部复制到剪贴板\t(&O)"); //"C&opy All to Clipboard"
	    AppendMenu(m, MF_ENABLED, IDM_CLRSB, "清除回滚\t(&L)"); //"C&lear Scrollback"
	    AppendMenu(m, MF_ENABLED, IDM_RESET, "重置终端\t(&T)"); //"Rese&t Terminal"
	    AppendMenu(m, MF_SEPARATOR, 0, 0);
	    AppendMenu(m, (conf_get_int(conf, CONF_resize_action)
			   == RESIZE_DISABLED) ? MF_GRAYED : MF_ENABLED,
		       IDM_FULLSCREEN, "全屏\t(&F)"); //"&Full Screen"
	    AppendMenu(m, MF_SEPARATOR, 0, 0);
	    if (has_help())
		AppendMenu(m, MF_ENABLED, IDM_HELP, "Help\t(&H)"); //""
	    str = dupprintf("About %s\t(&A)", appname); //""
    ...
    if (restricted_acl) {
	logevent(NULL, "以限制进程ACL运行"); //"Running with restricted process ACL"
    }
...
char *do_select(SOCKET skt, int startup)
{
    ...
    if (!hwnd)
	return "do_select(): 内部错误 (hwnd==NULL)"; //"internal error"
    if (p_WSAAsyncSelect(skt, hwnd, msg, events) == SOCKET_ERROR) {
	switch (p_WSAGetLastError()) {
	  case WSAENETDOWN:
	    return "网络已关闭"; //"Network is down"
	  default:
	    return "WSAAsyncSelect(): 未知错误"; //"unknown error"
	}
    }
    return NULL;
}
...
static void update_savedsess_menu(void)
{
	...
	AppendMenu(savedsess_menu, MF_GRAYED, IDM_SAVED_MIN, "(无会话)"); //"(No sessions)"
}
...
void update_specials_menu(void *frontend)
{
	...
	if (new_menu) {
	    InsertMenu(popup_menus[j].menu, IDM_SHOWLOG,
		       ...
		       (UINT_PTR) new_menu, "特殊命令\t(&P)"); //"S&pecial Command"
	    InsertMenu(popup_menus[j].menu, IDM_SHOWLOG,
...
void connection_fatal(void *frontend, const char *fmt, ...)
{
    ...
    va_end(ap);
    sprintf(morestuff, "%.70s 致命错误", appname); //"Fatal Error"
...
void cmdline_error(const char *fmt, ...)
{
    ...
    stuff = dupvprintf(fmt, ap);
    va_end(ap);
    sprintf(morestuff, "%.70s 命令行错误", appname); //"Command Line Error"
    MessageBox(hwnd, stuff, morestuff, MB_ICONERROR | MB_OK);
    sfree(stuff);
    exit(1);
}
...
void notify_remote_exit(void *fe)
{
	...
	if (close_on_exit == FORCE_ON ||
	    ...
	    if (exitcode != INT_MAX)
		MessageBox(hwnd, "远程主机关闭连接", //"Connection closed by remote host"
				...
				WPARAM wParam, LPARAM lParam)
{
      ...
      case WM_CLOSE:
	{
	    char *str;
	    show_mouseptr(1);
	    str = dupprintf("%s 退出确认", appname); //"Exit Confirmation"
	    if (session_closed || !conf_get_int(conf, CONF_warn_on_close) ||
		MessageBox(hwnd,
			   "您确定要关闭此会话?", //"Are you sure you want to close this session?"
...
void nonfatal(const char *fmt, ...)
{
    ...
    stuff = dupvprintf(fmt, ap);
    va_end(ap);
    sprintf(morestuff, "%.70s 错误", appname); //"Error"
    MessageBox(hwnd, stuff, morestuff, MB_ICONERROR | MB_OK);
    sfree(stuff);
}
...
void do_beep(void *frontend, int mode)
{
	...
	if (!p_PlaySound || !p_PlaySound(bell_wavefile->path, NULL,
	    ...
	    char otherbuf[100];
	    sprintf(buf, "未能播放音频文件\n%s\n" //"Unable to play sound file"
		    "改用默认声音", bell_wavefile->path);//"Using default sound instead"
	    sprintf(otherbuf, "%.70s 音频错误", appname);//"Sound Error"
