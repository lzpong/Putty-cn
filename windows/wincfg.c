			  ...
			  int midsession, int protocol)
{
    ...
    s = ctrl_getset(b, "Window", "scrollback",
		    "控制窗口中的滚动条");//"Control the scrollback in the window"
    ctrl_checkbox(s, "全屏模式下显示滚动条", 'i',//"Display scrollbar in full screen mode"
    ...
    s = ctrl_getset(b, "Terminal/Keyboard", "features",
		    "启用额外的键盘功能:");//"Enable extra keyboard features:"
    ctrl_checkbox(s, "AltGr作为组合键", 't',//"AltGr acts as Compose key"
		  HELPCTX(keyboard_compose),
		  conf_checkbox_handler, I(CONF_compose_key));
    ctrl_checkbox(s, "Control-Alt与AltGr不同", 'd',//"Control-Alt is different from AltGr"
	...
	for (i = 0; i < s->ncontrols; i++) {
		...
		c->radio.buttons[c->radio.nbuttons-1] =
		    dupstr("播放自定义声音文件");//"Play a custom sound file"
		c->radio.buttons[c->radio.nbuttons-2] =
		    dupstr("使用PC扬声器发出哔声");//"Beep using the PC speaker"
		c->radio.buttondata =
    ...
    ctrl_filesel(s, "自定义声音文件作为铃声播放:", NO_SHORTCUT,//"Custom sound file to play as a bell:"
		 FILTER_WAVE_FILES, FALSE, "选择铃声文件",//"Select bell sound file"
		 HELPCTX(bell_style),
     ...
     */
    ctrl_radiobuttons(s, "响铃时任务栏/标题指示:", 'i', 3,//"Taskbar/caption indication on bell:"
		      HELPCTX(bell_taskbar),
		      conf_radiobutton_handler,
		      I(CONF_beep_ind),
		      "禁用", I(B_IND_DISABLED),//"Disabled"
		      "闪烁", I(B_IND_FLASH),//"Flashing"
		      "不变", I(B_IND_STEADY), NULL);//"Steady"
    ...
    ctrl_checkbox(s, "凹边边缘(稍厚)", 's',//"Sunken-edge border (slightly thicker)"
    ...
    s = ctrl_getset(b, "Window/Appearance", "font",
		    "设置字体");//"Font settings"
    ctrl_checkbox(s, "允许选择可变间距字体", NO_SHORTCUT,//"Allow selection of variable-pitch fonts"
                  HELPCTX(appearance_font), variable_pitch_handler, I(0));
    ctrl_radiobuttons(s, "字体质量:", 'q', 2,//"Font quality:"
		      HELPCTX(appearance_font),
		      conf_radiobutton_handler,
		      I(CONF_font_quality),
		      "抗锯齿", I(FQ_ANTIALIASED),//"Antialiased"
		      "非抗锯齿", I(FQ_NONANTIALIASED),//"Non-Antialiased"
    ...
    ctrl_checkbox(s, "Caps Lock作为西里尔字符开关", 's',//"Caps Lock acts as Cyrillic switch"
    ...
    s = ctrl_getset(b, "Window/Translation", "trans",
		    "转换接收到的数据字的符集");//"Character set translation on received data"
    ctrl_text(s, "(此处未列出Windows支持的代码页,\n如许多系统上的CP866,可以手动输入)",
     ...
     * characters.
     */
    str = dupprintf("调整%s显示线条图字符", appname);//"Adjust how %s displays line drawing characters"
	...
	for (i = 0; i < s->ncontrols; i++) {
		...
		c->radio.buttons[c->radio.nbuttons-3] =
		    dupstr("字体具有XWindows编码");//"Font has XWindows encoding"
		c->radio.buttons[c->radio.nbuttons-2] =
		    dupstr("在ANSI和OEM模式下使用字体");//"Use font in both ANSI and OEM modes"
		c->radio.buttons[c->radio.nbuttons-1] =
		    dupstr("仅在OEM模式下使用字体");//"Use font in OEM mode only"
		c->radio.buttondata =
		    sresize(c->radio.buttondata, c->radio.nbuttons, intorptr);
    ...
    s = ctrl_getset(b, "Window/Selection", "format",
		    "粘贴字符格式");//"Formatting of pasted characters"
    ctrl_checkbox(s, "粘贴RTF到剪贴板以及纯文本", 'f',//"Paste to clipboard in RTF as well as plain text"
    ...
    s = ctrl_getset(b, "Window/Selection", "mouse",
		    "控制使用鼠标");//"Control use of mouse"
	ctrl_radiobuttons(s, "鼠标按键的动作:", 'm', 1,//"Action of mouse buttons:"
		HELPCTX(selection_buttons),
		conf_radiobutton_handler,
		I(CONF_mouse_is_xterm),
		"Windows(中键扩展,右键菜单)", I(2),//"Windows (Middle extends, Right brings up menu)"
		"折中(中键扩展,右键粘贴)", I(0),//"Compromise (Middle extends, Right pastes)"
		"xterm(右键扩展,中键粘贴)", I(1), NULL);//"xterm (Right extends, Middle pastes)"
    /*
    ...
    s = ctrl_getset(b, "Window/Colours", "general",
		    "颜色使用的常规选项");//"General options for colour usage"
    ctrl_checkbox(s, "尝试使用逻辑调色板", 'l',//"Attempt to use logical palettes"
		  HELPCTX(colours_logpal),
		  conf_checkbox_handler, I(CONF_try_palette));
    ctrl_checkbox(s, "使用系统色", 's',//"Use system colours"
     ...
     */
    s = ctrl_getset(b, "Window", "size", "设置窗口大小");//"Set the size of the window"
    ctrl_radiobuttons(s, "窗口大小调整时", 'z', 1,//"When window is resized:"
		      HELPCTX(window_resize),
		      conf_radiobutton_handler,
		      I(CONF_resize_action),
		      "更改行数和列数", I(RESIZE_TERM),//"Change the number of rows and columns"
		      "更改字体大小", I(RESIZE_FONT),//"Change the size of the font"
		      "仅最大化才更改字体大小", I(RESIZE_EITHER),//"Change font size only when maximised"
		      "禁止调整大小", I(RESIZE_DISABLED), NULL);//"Forbid resizing completely"
    ...
    ctrl_checkbox(s, "ALT-F4关闭窗口", '4',//"Window closes on ALT-F4"
		  HELPCTX(behaviour_altf4),
		  conf_checkbox_handler, I(CONF_alt_f4));
    ctrl_checkbox(s, "ALT-Space弹出系统菜单", 'y',//"System menu appears on ALT-Space"
		  HELPCTX(behaviour_altspace),
		  conf_checkbox_handler, I(CONF_alt_space));
    ctrl_checkbox(s, "ALT弹出系统菜单", 'l',//"System menu appears on ALT alone"
		  HELPCTX(behaviour_altonly),
		  conf_checkbox_handler, I(CONF_alt_only));
    ctrl_checkbox(s, "确保窗口总是在顶部", 'e',//"Ensure window is always on top"
		  HELPCTX(behaviour_alwaysontop),
		  conf_checkbox_handler, I(CONF_alwaysontop));
    ctrl_checkbox(s, "Alt-Enter全屏", 'f',//"Full screen on Alt-Enter"
	...
	for (i = 0; i < s->ncontrols; i++) {
		...
		c->generic.label = dupstr("Telnet命令,本地代理命令");//"Telnet command, or local proxy command"
		break;
	    }
	}
    }
	...
	ctrl_filesel(s, "本地显示X授权文件", 't',//"X authority file for local display"
		     NULL, FALSE, "选择X授权文件",//"Select X authority file"
